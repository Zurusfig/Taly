...
import { useCategories, useUpdateCategory, useArchiveCategory } from '@/hooks/useCategories';
...
export default function EditCategory() {
  ...
  async function handleSave() {
    if (!name.trim()) return;
    try {
      await updateCategory.mutateAsync({ id: category!.id, name: name.trim(), kind, color });
      router.back();
    } catch (e: any) {
  ...
  async function handleArchive() {
    Alert.alert('Archive category', 'The category will be hidden but transactions are kept.', [
  ...
  return (
    <View style={[styles.flex, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Button label="Save changes" onPress={handleSave} loading={updateCategory.isPending} disabled={!name.trim()} />
        <Button label="Archive category" variant="danger" onPress={handleArchive} loading={archiveCategory.isPending} />
      </ScrollView>
    </View>
  );
}
